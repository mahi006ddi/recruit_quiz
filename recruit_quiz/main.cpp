...
int main()
{
	...
	questions[2].q = to_string(x) + "ー(" + to_string(y*w) + "＋" + to_string(z*w) + ")÷" + to_string(w) + "の答えは？";
	...
	questions.push_back({
		"底面の面積" + to_string(x) + "cm、高さ" + to_string(y) + "cmの塩水がある。\n" + "この円錐の体積をXπcm^3とする。Xの値を求めよ",
		to_string(x * x * y / 3) });
	...
	questions.push_back({
		to_string(x) + "人のうち" + to_string(y) + "人をえらんでならべる方法は何通りあるか？",
		to_string(z) });
